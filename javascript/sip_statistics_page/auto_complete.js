...
$(document).ready(function(){
	...
	// $.post('controller/sip_statistics_page/autoComplete.php', {from:source, iorc:ip_addr}, function(data,status){
	// 	if(status == "success"){
	...
	// 	}
	// });
	// $.post('controller/sip_statistics_page/autoComplete.php', {from:destination, iorc:ip_addr}, function(data,status){
	// 	if(status == "success"){
	// 		var jData = JSON.parse(data);
	// 		for(var i=0; i<jData.length; i++){
	// 			var row = $("<option></option>")
