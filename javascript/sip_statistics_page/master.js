...
$(document).ready(function(){
	...
	function masterSearch(){
		...
		$.ajax({
			type: 'POST',
			url: 'controller/sip_statistics_page/master_post.php',
			dataType: 'json',
			data: data,
			success: function(data){
				for (var i = 0; i < data.length; i++) {
					var country_src = getCountryCode(data[i]["SRC"]);
					var country_dst = getCountryCode(data[i]["DST"]);
	                ...
	                row.append("<td><a title='" + country_src[0] + "'><div class='flag flag-" + country_src[1] + "'></div></a> " + data[i]["SRC"] + "</td>");
	                row.append("<td><a title='" + country_dst[0] + "'><div class='flag flag-" + country_dst[1] + "'></div></a> " + data[i]["DST"] + "</td>");
