...
function testAlert(from, to){
    ...
    //console.log(from + " | " + to);
    $.ajax({
        url: 'controller/archive_page/test_post.php',
        type: "POST",
    ...
    for (i = 0; i < jsonData.length; i += 1) {
        in_data.push({
            y:Number(jsonData[i]["TRUNK_IN"]),
            first:Number(jsonData[i]["TRUNK_FIRST"]),
            last:Number(jsonData[i]["TRUNK_LAST"])
        });
        out_data.push({
            y:Number(jsonData[i]["TRUNK_OUT"]),
            first:Number(jsonData[i]["TRUNK_FIRST"]),
            last:Number(jsonData[i]["TRUNK_LAST"])
        });
        in_attempt_data.push({
            y: Number(jsonData[i]["TRUNK_IN_ATTEMPT"]),
            first:Number(jsonData[i]["TRUNK_FIRST"]),
            last:Number(jsonData[i]["TRUNK_LAST"])
        });
        out_attempt_data.push({
            y:Number(jsonData[i]["TRUNK_OUT_ATTEMPT"]),
            first:Number(jsonData[i]["TRUNK_FIRST"]),
            last:Number(jsonData[i]["TRUNK_LAST"])
        });
        x_data.push(
