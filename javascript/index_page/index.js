...
function two_out_one(){
    ...
    if(global_data_length < 24){
        ...
        for (i = -24; i < 0; i += 1) {
            // console.log(String(jsonData[i+24]["TRUNK_DATE"]));
            data_out_attempt.push({
                // x: time + i * time_limit, //jsonData[i+30]["TRUNK_TIME"],
                // x: String(jsonData[i+24]["TRUNK_DATE"]),
...
function requestDataOut(){
    ...
    setInterval(function () {
        $.ajax({
            url: 'controller/index_page/out_updater.php',
            type: 'GET',
            dataType: 'json',
            async: false,
            success: function(data) {
