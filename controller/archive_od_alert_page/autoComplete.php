<?php
	...
	if($whichOne == "src" && $iorc == "ip_addr"){
		$compiled = oci_parse($conn, "SELECT DISTINCT ARG1 FROM OD_ALERT ORDER BY ARG1 ASC");
	}else if($whichOne == "dst" && $iorc == "ip_addr"){
		$compiled = oci_parse($conn, "SELECT DISTINCT ARG2 FROM OD_ALERT ORDER BY ARG2 ASC");
	}else if($whichOne == 'type' && $iorc == 'alert_type'){
		$compiled = oci_parse($conn, "SELECT DISTINCT TYPE FROM OD_ALERT ORDER BY TYPE ASC");
	}
