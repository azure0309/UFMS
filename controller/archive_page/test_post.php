	...
	if($data[0]["DIFF"] > 300 && $data[0]["DIFF"] <= 10800){
		...
		if($howMany > count($data)){
			for($i = 0; $i< $howMany - count($data); $i++){
				array_push($returnArray, array('TRUNK_FIRST' => null, 'TRUNK_LAST' => null ,'TRUNK_IN' => null, 'TRUNK_OUT' => null, 'TRUNK_DATE' => null, 'TRUNK_IN_ATTEMPT' => null, 'TRUNK_OUT_ATTEMPT' => null));
			}
			for($i = $howMany - count($data); $i < count($data); $i++){
				array_push($returnArray, array('TRUNK_FIRST' => $data[$i]["TRUNK_ID"], 'TRUNK_LAST' => $data[$i]["TRUNK_ID"] ,'TRUNK_IN' => $data[$i]["TRUNK_IN"], 'TRUNK_OUT' => $data[$i]["TRUNK_OUT"], 'TRUNK_DATE' => $data[$i]["TRUNK_DATE"], 'TRUNK_IN_ATTEMPT' => $data[$i]["TRUNK_IN_ATTEMPT"], 'TRUNK_OUT_ATTEMPT' => $data[$i]["TRUNK_OUT_ATTEMPT"]));
			}
		}
		...
		if($howMany < count($data)){
			for($i = count($data) - $howMany; $i < count($data); $i++){
				...
				$out_attempt_counter += $data[$i]["TRUNK_OUT_ATTEMPT"];
				if($i%2 == 0 || $i == count($data) - 1){
					$last_id = $data[$i]["TRUNK_ID"];
					array_push($returnArray, array('TRUNK_FIRST' => $first_id, 'TRUNK_LAST' => $last_id, 'TRUNK_IN' => $in_counter, 'TRUNK_OUT' => $out_counter, 'TRUNK_DATE' => $data[$i]["TRUNK_DATE"], 'TRUNK_IN_ATTEMPT' => $in_attempt_counter, 'TRUNK_OUT_ATTEMPT' => $out_attempt_counter));
					$in_counter = 0;
					$out_counter = 0;
					$in_attempt_counter = 0;
					$out_attempt_counter = 0;
